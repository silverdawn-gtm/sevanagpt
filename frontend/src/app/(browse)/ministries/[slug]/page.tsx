...
import type { Ministry, SchemeListItem } from "@/lib/types";
...
export default function MinistryDetailPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    getMinistry(params.slug, language, page, PAGE_SIZE)
      .then((data) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <nav className="text-sm text-gray-500 mb-6">
        <Link href="/ministries" className="hover:text-emerald-600">{t("browse.by_ministry")}</Link>
        <span className="mx-2">/</span>
        <span className="text-gray-900">{ministry.name}</span>
      </nav>

      <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">{ministry.name}</h1>
